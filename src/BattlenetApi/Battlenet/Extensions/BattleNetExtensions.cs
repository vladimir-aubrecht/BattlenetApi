...
namespace ASoft.BattleNet.Battlenet.Extensions
{
    public static class BattleNetExtensions
    {
        ...
        {
            return battleNetClient.QueryBattleNetApiAsync<OAuthToken>(HttpMethod.Get, $"/oauth/authorize?access_type=online&client_id={clientId}&redirect_uri={redirectUrl}&response_type=code&state=", null, cookies, null);
        }
        ...
        {
            var tokenMetadata = await battleNetClient.VerifyTokenAsync(accessToken);
